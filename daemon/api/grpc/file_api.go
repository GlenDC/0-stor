...
func (service *fileService) Write(ctx context.Context, req *pb.WriteRequest) (*pb.WriteResponse, error) {
	...
	if err != nil {
		return nil, mapClientAndMetastorError(err)
	}
...
func (service *fileService) WriteStream(stream pb.FileService_WriteStreamServer) error {
	ctx := stream.Context()
	key, err := extractStringFromContext(ctx, rpctypes.MetaKeyTag)
	if err != nil {
		return err
	}
	...
	group.Go(func() error {
		var err error
		metadata, err = service.client.Write([]byte(key), reader)
		return mapClientAndMetastorError(err)
	})
